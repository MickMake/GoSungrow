...
func generateTypes(obj map[string]interface{}, structName string, tags []string, depth int, subStructMap map[string]string, convertFloats bool) string {
	...
	for _, key := range keys {
		...
		valueType = TypeLookup(valueType)
...
func typeForValue(value interface{}, structName string, tags []string, subStructMap map[string]string, convertFloats bool) string {
	...
	for range Only.Once {
		...
		if objects, ok := value.([]interface{}); ok {
			...
			if len(types) == 1 {
				v = "[]" + typeForValue(mergeElements(objects).([]interface{})[0], structName, tags, subStructMap, convertFloats)
				break
			}
			v = NameArrayInterface
			break
		}

		if object, ok := value.(map[interface{}]interface{}); ok {
			v = generateTypes(convertKeysToStrings(object), structName, tags, 0, subStructMap, convertFloats) + "}"
			break
		}

		if object, ok := value.(map[string]interface{}); ok {
			v = generateTypes(object, structName, tags, 0, subStructMap, convertFloats) + "}"
			break
		}
		...
		if v == NameFloat64 && convertFloats {
