...
func (pm *PointsMap) Resolve(point string) *Point {
	if ret, ok := pm.Map[PointId(point)]; ok {
		return &ret
	}
	return nil
}
...
func (pm *PointsMap) GetDevicePoint(devicePoint string) *Point {
	...
	for range Only.Once {
		...
		ret = pm.Get(s[0], PointId(s[1]))
	}
	return ret
}

func (pm *PointsMap) Exists(pid PointId) bool {
	var ok bool
	for range Only.Once {
...
func (pm *PointsMap) Add(pid PointId, point Point) bool {
	var ok bool
	for range Only.Once {
		if !pm.Exists(point.Id) {
			// point.Parent = ParentDevice{}
		...
		jp, _ := json.Marshal(point)
		if string(jep) == string(jp) {
			break
		}

		if strings.ToLower(ep.Name) == string(ep.Id) {
			ep.Name = point.Name
		}
		if ep.Name == "" {
			ep.Name = point.Name
		...
		// if string(jep) == string(jep2) {
		// 	break
		// }
		if ep.HasNotChanged(pm.Map[point.Id]) {
			break
		}
		fmt.Printf("EXISTS[%s]:\n", pid)
