...
func (p *SunGro) CountDialRules(domain string) error {

	for range Only.Once {
		domain = p.VerifyDomain(domain)

		query, err := p.Areas.DialRule.Count(domain)
		if err != nil {
			p.Error = err
			break
		}
...
func (p *SunGro) ListDialRules(domain string) error {

	for range Only.Once {
		domain = p.VerifyDomain(domain)

		query, err := p.Areas.DialRule.List(domain)
		if err != nil {
			p.Error = err
			break
		}
...
func (p *SunGro) ReadDialRules(domain string) error {

	for range Only.Once {
		domain = p.VerifyDomain(domain)

		query, err := p.Areas.DialRule.Read(domain)
		if err != nil {
			p.Error = err
			break
		}
