...
func (t *Device) SetBaseUrl(u string) error {
	for range Only.Once {
		if u == "" {
...
func (t *Device) Count(domain string) Count {
	for range Only.Once {
...
func (t *Device) Create(domain string, device string, user string, mac string, model string) Create {
	for range Only.Once {
...
func (t *Device) Delete(device string, match string) Delete {
	for range Only.Once {
		t.delete.Request = DeleteRequest{
			Device:           device,
			TerminationMatch: match,
		}
...
func (t *Device) List(domain string) List {
	for range Only.Once {
...
func (t *Device) Read(domain string) Read {
	for range Only.Once {
...
func (t *Device) Update(device string) Update {
	for range Only.Once {
