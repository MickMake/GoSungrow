...
func (p *SunGro) CountMeetings(domain string) error {

	for range Only.Once {
		domain = p.VerifyDomain(domain)

		query, err := p.Areas.Meeting.Count(domain)
		if err != nil {
			p.Error = err
			break
		}
...
func (p *SunGro) ListMeetings(domain string) error {

	for range Only.Once {
		domain = p.VerifyDomain(domain)

		query, err := p.Areas.Meeting.List(domain)
		if err != nil {
			p.Error = err
			break
		}
...
func (p *SunGro) ReadMeetings(domain string) error {

	for range Only.Once {
		domain = p.VerifyDomain(domain)

		query, err := p.Areas.Meeting.Read(domain)
		if err != nil {
			p.Error = err
			break
		}
